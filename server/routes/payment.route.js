    ...
    router.get('/config', (req, res) => {
        try {
            ...
            const clientId = process.env.PAYPAL_CLIENT_ID; 
            if (!clientId) {
                return res.status(500).json({ success: false, error: 'PayPal Client ID not configured.' });
            }
            res.json({ clientId });
        } catch (error) {
            console.error('Error fetching PayPal client ID:', error);
            res.status(500).json({ success: false, error: 'Internal server error fetching PayPal config.' });
        }
    });
    ...
    router.post('/capture-order', async (req, res) => {
        try {
            ...
            if (captureDetails.status === 'COMPLETED') {
                ...
                const purchaseUnit = captureDetails.purchase_units && captureDetails.purchase_units.length > 0
                    ? captureDetails.purchase_units[0]
                    : null;

                const paymentCapture = purchaseUnit && purchaseUnit.payments && purchaseUnit.payments.captures && purchaseUnit.payments.captures.length > 0
                    ? purchaseUnit.payments.captures[0]
                    : null;

                if (!purchaseUnit || !purchaseUnit.amount || !paymentCapture) {
                    console.error('PayPal Capture Order Error: Missing expected details in capture response', captureDetails);
